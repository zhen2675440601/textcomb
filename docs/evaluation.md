...
2. 评测记录文章场景（general、academic、financial）和长度区间；论文的公式/引用/DOI、财报的金额/比例/日期单独统计保护区误报。保护区候选可以保留为疑似，但不得进入正式问题准确率。
3. 两人不一致的项目交给第三人裁决。
4. `char_start` 与 `char_end` 使用提取后不可变正文的 Unicode 字符下标，范围为左闭右开；重叠上下文区不单独计为预测范围。
5. 同一处只保留一个裁决问题；问题类别必须唯一。分段建议不进入正式问题准确率承诺。
6. 每次评测锁定模型、提示词、依据库和分析器版本，并保存服务商返回的用量，不保存完整提示词日志。
