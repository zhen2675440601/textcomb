...
只有发布说明列明的参考模型组合可以附带质量指标。当前社区构建没有激活任何参考评测范围，因此即使数据库中的模型配置带有 `is_reference`，接口和报告仍会显示“未经验证”。将来激活参考范围时必须同时锁定模型、提示词版本、分析器版本、文章场景和最大字数；用户自带模型不能沿用参考模型的准确率与召回率承诺。

## 提示词版本

报告中的 `zh-cn-proofread-v4` 是当前内置初检与复核提示词的版本标识。任务创建时会把场景提示一并写入模型快照，并把报告版本标记为例如 `zh-cn-proofread-v4+scenario-v1-academic`，以便重试和后续部署仍使用同一组指令。社区 MVP 暂未提供提示词管理接口或页面；不要直接修改数据库中的提示词记录，因为迁移会重新写入当前内置版本。
